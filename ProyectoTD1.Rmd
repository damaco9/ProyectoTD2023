...
-   <div>
    ...
    -   <div>
        ...
        [3.1 Preguntas Iniciales.](#3.1.-Preguntas-Iniciales:)
...
```{r eval=T, include=F}
rm(list=ls())
packages = c("knitr", "ggplot2","tidyr","dplyr","readr","tidyverse", "plotly","broom","kableExtra")
...
Una vez creado, usamos *ggplot* y a su vez *facet_wrap* para crear el gráfico con la trayectoria de todas las variables respecto al tempo de dicho subconjunto.
Por último, con la libreria `plotly` mostramos el garáfico interactivo.
...
Un posible método de imputación para todos estos datos podría ser sacar la media entre el valor anterior y posterior al del dato anómalo. Pero en la variable TOCO de *1059xlxveei79_ECGf2* no valdría la pena imputar ya que estos datos anómalos podrían ser causa de que los datos de esta pacienta difieren mucho a los de las otras.
...
Como hemos comentado ya, los datos de la paciente *1059xlxveei79_ECGf2* son extrañamente diferentes a las otras 2 pacientes. Esto puede deberse a diferentes posibles causas. Pero uno en que hemos pensado, sabiendo el tipo de datos que estamos tratando, es que el feto de esta pacienta está mucho más desarrollado que los otros. LLeva más tiempo de gestación y por eso los valores se parecen más a los de una persona común.
...
En los ficheros digitales vemos lo siguiente, y es que en el puento 103-104 las variables Pd, Pm y Ps dan un salto y pasan de valer 0 a valer 94, 118 y 155 respectivamente. La variable SPO hace lo mismo en 236-236.25, que pasa a valer 98. Luego vemos que TOCo toma valores mucho más variados que en las otras dos pacientes y HR2 varía mucho.

No podemos saber con seguridad, a partir de la informacion que tenemos el motivo de estas variaciones, pero la tesis de que la paciente lleva más semanas de gestación explicaría en parte estas diferencias; ya que indicaria que el bebé está mucho más desarrollado y se pueden tomar más medidas de él porque responde más intensamente a estímulos externos.
...
outMHR <- which(datos_fich_dig$MHR > mean(datos_fich_dig$MHR) + 1.5 * sd(datos_fich_dig$MHR) | 
        datos_fich_dig$MHR < mean(datos_fich_dig$MHR) - 1.5 * sd(datos_fich_dig$MHR) |          datos_fich_dig$MHR == 16)
for (i in outMHR){
